...
data_path = os.path.join(os.environ['HOME'], 'TEMPLATE_AIRFOILS_SA', 'ref_features_with_clusters.npy')
data = np.load(data_path)
ref_features = data[:, :-1]  # Coordinate x, y della mesh di riferimento
ref_cluster_idx = data[:, -1].astype(int)  # Indici dei cluster
print('clustering...')
...
kdtree = cKDTree(ref_features)

_, idx = kdtree.query(features)
cluster_idx = ref_cluster_idx[idx]

#data_to_save = np.column_stack((features, cluster_idx))
#np.save('prova_features_with_clusters.npy', data_to_save)
